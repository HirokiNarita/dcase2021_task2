...
class DCASE2021_task2_Dataset(Dataset):
    def __init__(self, dataset_path=CONFIG['IO_OPTION']['INPUT_ROOT'], class_name='ToyCar', phase='train'):
        ...
        if phase == 'train':
            self.wav_names = self.wav_names[:1003]
        else:
    ...
    def load_dataset_folder(self):
        if self.phase == 'train':
            ...
            wav_names = [f"{wav_dir}/{file}" for file in wav_types]
        else:
            wav_dir_src = os.path.join(self.dataset_path, self.class_name, 'source_test')
            wav_types_src = sorted(os.listdir(wav_dir_src))
            wav_types_src = [f"{wav_dir_src}/{file}" for file in wav_types_src]
            
            wav_dir_tgt = os.path.join(self.dataset_path, self.class_name, 'target_test')
            wav_types_tgt = sorted(os.listdir(wav_dir_tgt))
            wav_types_tgt = [f"{wav_dir_tgt}/{file}" for file in wav_types_tgt]
            
            wav_names = wav_types_src + wav_types_tgt
