...
# extract function
def train_net(net, dataloaders_dict, optimizer, scheduler, num_epochs, writer, model_out_path, score_out_path, pred_out_path):
    # make img outdir
    ...
    for epoch in range(num_epochs):
        ...
        # early stopping
        if best_pAUC < val_pAUC_hmean:
            best_score = all_scores_df.copy()
            best_tr_losses = tr_losses
            best_pAUC = val_pAUC_hmean.copy()
    ...
    best_log = (
                ...
                f" val_pAUC_hmean:{best_pAUC:.6f},"
                )
    logger.info(best_log)
    display(best_score)
    output_dicts = {'best_epoch':best_epoch, 'best_pAUC':best_pAUC, 'best_pred':best_pred}
    return output_dicts
