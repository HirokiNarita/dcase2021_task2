...
def train_net(net, dataloaders_dict, optimizer, num_epochs, writer, model_out_path, score_out_path, pred_out_path):
    ...
    for epoch in range(num_epochs):
        ...
        # early stopping
        if best_criterion < val_AUC_hmean:
            best_score = all_scores_df.copy()
            best_tr_losses = tr_losses
            best_criterion = val_AUC_hmean.copy()
    ...
    best_log = (
                ...
                f" val_AUC_hmean:{best_criterion:.6f},"
                )
    logger.info(best_log)
    display(best_score)
    output_dicts = {'best_epoch':best_epoch, 'best_AUC':best_criterion, 'best_pred':best_pred}
    return output_dicts
