...
def extract_net(net, dataloaders_dict):
    ...
    for phase in ['train', 'valid_source', 'valid_target']:
        ...
        labels = []
        wav_names = []
        for cnt, sample in enumerate(tqdm(dataloaders_dict[phase])):
            ...
            with torch.no_grad():
                if cnt == 0 and phase == 'train':
                    output_dict = net(input, device, first_flag=True)  # (batch_size,input(2D)) 
                else:
                    output_dict = net(input, device, first_flag=False)
