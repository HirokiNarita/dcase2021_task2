...
def extract_net(net, dataloaders_dict):
    ...
    for i in range(len(net.layer1)):
        net.layer1[i].register_forward_hook(hook)
        net.layer2[i].register_forward_hook(hook)
        net.layer3[i].register_forward_hook(hook)
        net.layer4[i].register_forward_hook(hook)
